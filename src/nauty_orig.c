...
#if !MAXN
static int firstpathnode0(int*, int*, int, int, tcnode*, int, int);
static int othernode0(int*, int*, int, int, tcnode*, int, int);
#else
static int firstpathnode(int*, int*, int, int, int, int);
static int othernode(int*, int*, int, int, int, int);
#endif
...
void non_node_visualize(int *lab, int *ptn, int level, int tc, int tv, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        // if (i > 0) printf(", ");
        if (ptn[i] >= NAUTY_INFINITY)
            printf("-   ");
        else
            printf("%-4d", ptn[i]);
    }        
    printf("\nLV: %d  TC: %d  TV: %d\n", level, tc, tv);
    printf("EQL_F: %d   COMP_C: %d   EQL_C: %d\n", eqlev_first, comp_canon, eqlev_canon);
}
...
#if !MAXN
    retval = firstpathnode0(lab,ptn,1,numcells,&tcnode0, 0,0);
#else   
    retval = firstpathnode(lab,ptn,1,numcells, 0, 0);
#endif  
...
firstpathnode0(int *lab, int *ptn, int level, int numcells,
          tcnode *tcnode_parent, int this_tc, int this_tv)
#else
firstpathnode(int *lab, int *ptn, int level, int numcells, int this_tc, int this_tv)
#endif
{
    int tv;
    ...
    if (__DEBUG__) printf("firstpathnode called at level %d  refcode %d   numcells %d\n",level, (short) refcode, numcells);
    if (__DEBUG__) {
        printf("numnodes: %lu\n[]\n", stats->numnodes);
        non_node_visualize(lab, ptn, level, this_tc, this_tv, n);
        ...
        if (orbits[tv] == tv)   /* ie, not equiv to previous child */
        {
            ...
            if (tv == tv1)
            {
                ...
                rtnlevel = firstpathnode0(lab,ptn,level+1,numcells+1,
                                         tcnode_this, tc, tv);
#else
                rtnlevel = firstpathnode(lab,ptn,level+1,numcells+1, tc, tv);
#endif
                childcount = 1;
                gca_first = level;
                stabvertex = tv1;
            ...
            else
            {
                ...
                rtnlevel = othernode0(lab,ptn,level+1,numcells+1,
                                     tcnode_this, tc, tv);
#else
                rtnlevel = othernode(lab,ptn,level+1,numcells+1, tc, tv);
#endif
                ++childcount;
            }
...
othernode0(int *lab, int *ptn, int level, int numcells,
      tcnode *tcnode_parent, int this_tc, int this_tv)
#else
othernode(int *lab, int *ptn, int level, int numcells, int this_tc, int this_tv)
#endif
{
    int tv;
    ...
    if (__DEBUG__) {
printf("level: %d   ccode: ", level); for (int i = 0; i < 10; ++i) printf(" %d", canoncode[i]); printf(" code: %d\n", code);
        printf("numnodes: %lu\n[]\n", stats->numnodes);
        non_node_visualize(lab, ptn, level, this_tc, this_tv, n);
        printf("noncheaplevel: %d\n", noncheaplevel);
        printf("\n");
   ...
   /* If children will be required, find new target cell and set tc to its
                                    ...
                                    tv = nextelement(tcell,M,tv))
    {
        ...
        ADDELEMENT(fixedpts,tv);
#if !MAXN   
        rtnlevel = othernode0(lab,ptn,level+1,numcells+1,tcnode_this, tc, tv);
#else
        rtnlevel = othernode(lab,ptn,level+1,numcells+1, tc, tv);
#endif
        DELELEMENT(fixedpts,tv);
...
firstterminal(int *lab, int level)
{
    ...
    if (getcanon)
    {
        ...
        if (__DEBUG__) printf("-=-=-=-=-=-=-=-= New canon BSF Found =-=-=-=-=-=-=-=-\n");
        comp_canon = 0;
        samerows = 0;
